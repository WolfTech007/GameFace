...
declare global {
  ...
  var __facepongQueue: WaitingEntry[] | undefined;
  // eslint-disable-next-line no-var
  var __facepongResults: Map<string, MatchInfo> | undefined;
}

export const dynamic = "force-dynamic";

function getQueue(): WaitingEntry[] {
  if (!globalThis.__facepongQueue) globalThis.__facepongQueue = [];
  return globalThis.__facepongQueue;
}

function getResults(): Map<string, MatchInfo> {
  if (!globalThis.__facepongResults) globalThis.__facepongResults = new Map();
  return globalThis.__facepongResults;
}
...
export async function POST(req: Request) {
  try {
    ...
    while (queue.length >= 2) {
      ...
      const peerRoomId = `facepong-${roomSuffix}`;
