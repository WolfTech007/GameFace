...
declare global {
  ...
  var __facecardQueue: WaitingEntry[] | undefined;
  // eslint-disable-next-line no-var
  var __facecardResults: Map<string, MatchInfo> | undefined;
}

export const dynamic = "force-dynamic";

function getQueue(): WaitingEntry[] {
  if (!globalThis.__facecardQueue) globalThis.__facecardQueue = [];
  return globalThis.__facecardQueue;
}

function getResults(): Map<string, MatchInfo> {
  if (!globalThis.__facecardResults) globalThis.__facecardResults = new Map();
  return globalThis.__facecardResults;
}
...
export async function POST(req: Request) {
  try {
    ...
    while (queue.length >= 2) {
      ...
      const peerRoomId = `facecard-${roomSuffix}`;
