...
declare global {
  ...
  var __lipreaderQueue: WaitingEntry[] | undefined;
  // eslint-disable-next-line no-var
  var __lipreaderResults: Map<string, MatchInfo> | undefined;
}

export const dynamic = "force-dynamic";

function getQueue(): WaitingEntry[] {
  if (!globalThis.__lipreaderQueue) globalThis.__lipreaderQueue = [];
  return globalThis.__lipreaderQueue;
}

function getResults(): Map<string, MatchInfo> {
  if (!globalThis.__lipreaderResults) globalThis.__lipreaderResults = new Map();
  return globalThis.__lipreaderResults;
}
...
export async function POST(req: Request) {
  try {
    ...
    while (queue.length >= 2) {
      ...
      const peerRoomId = `lipreader-${roomSuffix}`;
