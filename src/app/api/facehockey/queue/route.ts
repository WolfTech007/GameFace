...
declare global {
  ...
  var __facehockeyQueue: WaitingEntry[] | undefined;
  // eslint-disable-next-line no-var
  var __facehockeyResults: Map<string, MatchInfo> | undefined;
}

export const dynamic = "force-dynamic";

function getQueue(): WaitingEntry[] {
  if (!globalThis.__facehockeyQueue) globalThis.__facehockeyQueue = [];
  return globalThis.__facehockeyQueue;
}

function getResults(): Map<string, MatchInfo> {
  if (!globalThis.__facehockeyResults) globalThis.__facehockeyResults = new Map();
  return globalThis.__facehockeyResults;
}
...
export async function POST(req: Request) {
  try {
    ...
    while (queue.length >= 2) {
      ...
      const peerRoomId = `facehockey-${roomSuffix}`;
